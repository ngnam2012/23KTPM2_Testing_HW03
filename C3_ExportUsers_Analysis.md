...
| 2   | Màn hình Web (Phản hồi hệ thống)        | Sau khi bấm nút Export, **không có Toast Notification** hay thông báo nào trên UI ("Export thành công" hoặc "Đang xử lý..."). Người dùng chỉ nhận biết qua popup download mặc định của trình duyệt web. Nếu file lớn tải lâu, admin có thể tưởng hệ thống bị đơ và bấm nhiều lần.                                                                                                                    | **Nielsen #1** (Visibility of System Status), **Norman #2** (Feedback), **Shneiderman #3** (Informative Feedback)              | **2**                  | Ngay khi bấm Export: hiện Toast notification hoặc đổi state nút Export thành Loading (spinner + "Exporting..."). Khi xong: hiện Toast "Export thành công".                                            | GUI-37, GUI-40, GUI-46 |
...
| 7   | File Excel (Định dạng dữ liệu ô)        | Dữ liệu cột `Phone` và `Card Code` có chứa cả số nguyên (`23127514`) và chuỗi (`G3A81DE42`). Nhìn vào ảnh, các ô chứa số đang bị **căn lề phải** (Excel đang hiểu là dạng Number). Điều này rất nguy hiểm vì có thể làm mất số `0` ở đầu (dù phone `0901234567` đang giữ được số 0 nhưng cơ chế parse tự động của Excel có thể lỗi trên các phiên bản khác nhau hoặc bị đổi thành số khoa học `E+`). | **Nielsen #5** (Error Prevention), **Shneiderman #5** (Prevent Errors)                                                         | **1**                  | Trong code backend sinh file Excel, cần ép kiểu (force format) cột Phone và Member Code dưới định dạng **Text (String)** tuyệt đối, để ô luôn canh lề trái và không tự động xóa số `0` ở đầu.         | GUI-46                 |
| 8   | File Excel (Xử lý tên tách dòng)        | Trên Web hiển thị `Test Test`, trong Excel đã tách ra `Last Name` (Test) và `First Name` (Test). Cách xử lý này tốt cho việc sort/filter theo tên. Tuy nhiên, nếu user nhập "Nguyễn Minh Luân" vào ô full name thì thuật toán tách chuỗi có thể sai (VD lấy Luân làm First, Nguyễn Minh làm Last).                                                                                                   | **Nielsen #2** (Match Between System & Real World)                                                                             | **0**                  | (Chỉ là nhận xét, không tính là lỗi nếu form gốc C2 đã tách riêng 2 trường First Name và Last Name từ đầu). Cần đảm bảo data đồng nhất.                                                               | N/A                    |
...
| GUI-40       | Progress bar / Indicator                          | **N/A**    | Không có dấu hiệu về loading state khi đang export (dù là tooltip hay spinner). Cần test thực tế với data lớn. Xem Finding #2. |
| GUI-42       | Thông báo lỗi server                              | **N/A**    | Cần mô phỏng ngắt kết nối khi bấm Export để xem hệ thống xử lý lỗi tải file ra sao.                                            |
...
| **1 — Cosmetic**      | Cosmetic problem only   | **3**     | #1 (Tên file Timestamp), #6 (Thiếu cột Updated), #7 (Format number Excel)                 |
| **0 — Not a problem** | Nhận xét chung          | **1**     | #8                                                                                        |
